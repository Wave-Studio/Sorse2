...
import { SorsePolygonOpts } from "../../index";
...
export class Polygon extends SorseShapeCore {
	...
	public render(ctx: CanvasRenderingContext2D, x: number, y: number) {
		...
		ctx.beginPath();
		ctx.moveTo(x + this._points[0].x + this._position.x, y + this._points[0].y + this._position.y);
		for (let i = 1; i < this._points.length; i++) {
			ctx.lineTo(x + this._position.x + this._points[i].x, y + this._points[i].y + this._position.y);
		}
		ctx.closePath();
		ctx.fill();
	}
